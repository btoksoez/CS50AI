...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    """
    # Probabilities of inheritance given genes of parent
    combs = {
        0: PROBS["mutation"],
        1: 0.5,
        2: 1 - PROBS["mutation"]
    ...
    for person in people:
        ...
        # if person does not have parents, use unconditional probabilities
        if not mother and not father:
            person_prob = PROBS["gene"][person_gene]

        # else calculate based on combinations of parents
        else:
            if person_gene == 2:
                person_prob = combs[mother_genes] * combs[father_genes]
            elif person_gene == 1:
                person_prob = combs[mother_genes] * (1 - combs[father_genes]) + (1 - combs[mother_genes]) * combs[father_genes]
            elif person_gene == 0:
                person_prob = (1 - combs[mother_genes]) * (1 - combs[father_genes])

        # add trait probability given gene probability
        person_prob *= PROBS["trait"][person_gene][person_trait]

        # add to join probability
